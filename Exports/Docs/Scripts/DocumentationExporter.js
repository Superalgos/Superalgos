...
exports.documentationExporter = function documentationExporter() {
    ...
    async function render() {
        ...
        async function buildHtmlPage() {
            ...
            HTML = HTML + addDefinitionTable(thisObject.docsSchemaDocument.definition, 'definition-editable-', thisObject.currentDocumentBeingRendered.category, thisObject.currentDocumentBeingRendered.project, thisObject.currentDocumentBeingRendered.type, 0)
            ...
            */
             ...
             * @return {string}
             */
            function addTranslationIcons(translations) {
                let html = '<div id="translation-list"><span class="docs-language-selection">Select your language:</span>'
             ...
             * @returns {string}
             */
            function addDefinitionTable(definition, idPrefix, category, project, type, count) {
                if(definition.translations === undefined || definition.translations.length === 0) {
                    return addDefinitionTableForLanguage(definition, idPrefix, category, project, type, count, ED.DEFAULT_LANGUAGE)
                }
                const definitions = []
                const translations = [ED.DEFAULT_LANGUAGE].concat(definition.translations.map(t => t.language))
                for(let i = 0; i < translations.length; i++) {
                    definitions.push(addDefinitionTableForLanguage(definition, idPrefix, category, project, type, count, translations[i]))
                }
             ...
             * @returns {string}
             */
            function addDefinitionTableForLanguage(definition, idPrefix, category, project, type, count, language) {
                let html = ''
                ...
                */
                const languageClass = language + '-translation translation' + (count > 0 ? ' hidden' : '')
            ...
            async function addContent() {
                ...
                async function addIncludedParagraphs(includeText) {
                    ...
                    if(definition === true) {
                        html = html + addDefinitionTable(includedSchemaDocument, 'definition-included-', category, project, type, 0)
                    } else {
