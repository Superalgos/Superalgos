function newPlotter() {
    ...
    let slotHeight = (UI.projects.foundations.spaces.chartingSpace.viewport.visibleArea.bottomRight.y - UI.projects.foundations.spaces.chartingSpace.viewport.visibleArea.topLeft.y) / 10  // This is the amount of slots available
    ...
    function onMouseOver(event) {
        ...
        minUserPositionRate = UI.projects.foundations.utilities.dateRateTransformations.getRateFromPointAtBrowserCanvas(minPositionPoint, thisObject.container, coordinateSystem)

        let maxPositionPoint = {
            x: event.x,
            y: event.y - rateRange
        }
        maxUserPositionRate = UI.projects.foundations.utilities.dateRateTransformations.getRateFromPointAtBrowserCanvas(maxPositionPoint, thisObject.container, coordinateSystem)
    }
    ...
    function plot() {
        try {
            ...
            for (let i = 0; i < records.length; i++) {
                let record = records[i] //records[records.length - 1 - i] // NOTE: Order intentionally reverted to prevent pannels showing the last record which overlap / shadow all the others (think episodes in daily)
                                    ...
                                    */
                                    ...
                                    for (let i = 0; i < rateArray.length; i++) {
                                        ...
                                        */
                                        ...
                                        rate = UI.projects.foundations.utilities.dateRateTransformations.getRateFromPointAtBrowserCanvas(point, thisObject.container, coordinateSystem)
    ...
    function checkOutOfScreen(i, record) {
        ...
        */
        ...
        beginPoint = UI.projects.foundations.utilities.coordinateTransformations.transformThisPoint(beginPoint, thisObject.container)
        endPoint = UI.projects.foundations.utilities.coordinateTransformations.transformThisPoint(endPoint, thisObject.container)

        beginPoint = UI.projects.foundations.spaces.chartingSpace.viewport.fitIntoVisibleArea(beginPoint)
        endPoint = UI.projects.foundations.spaces.chartingSpace.viewport.fitIntoVisibleArea(endPoint)

        beginPoint = thisObject.fitFunction(beginPoint)
        endPoint = thisObject.fitFunction(endPoint)

        if (endPoint.x < UI.projects.foundations.spaces.chartingSpace.viewport.visibleArea.bottomLeft.x || beginPoint.x > UI.projects.foundations.spaces.chartingSpace.viewport.visibleArea.bottomRight.x) {
            return false
        } else {
            return true
        }
                ...
                for (let j = 0; j < chartPoints.points.length; j++) {
                    ...
                    if (point.pointFormula !== undefined) {
                        ...
                        */
                        ...
                        dataPoint = UI.projects.foundations.utilities.coordinateTransformations.transformThisPoint(dataPoint, thisObject.container)
                        let testPoint = {
                            x: dataPoint.x,
                            y: dataPoint.y
                        }
    ...
    function onDragFinished() {
        if (UI.projects.foundations.spaces.chartingSpace.visible === true) {
            recalculate()
        }
    }
