    ...
    function physics() {
        ...
        if (thisObject.isConnected() !== true) {
                ...
                if (networkNode !== undefined) {
                    if (networkNode.payload !== undefined) {
                        if (networkNode.payload.uiObject !== undefined) {
                            networkNode.payload.uiObject.setErrorMessage('Failed to Connect to Superalgos Platform Client via WebSockets. Retrying in 1 minute. Check at this node config if the host property has the IP address of the computer running Superalgos Platform Client.')
                        }
                    }
                }
            }
            ...
            if (networkNode !== undefined) {
                if (networkNode.payload !== undefined) {
                    if (networkNode.payload.uiObject !== undefined) {
                        networkNode.payload.uiObject.setStatus('Connected to Superalgos Platform Client via WebSockets.')
