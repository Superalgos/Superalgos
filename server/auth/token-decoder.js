...
function decodeToken (token, callBackFunction) {
  try {
    if (global.INFO_LOG === true) { logger.info('[INFO] ' + MODULE_NAME + ' -> decodeToken -> Entering function.') }
    ...
    function onKeyReady (err, key) {
      ...
      function onVerified (err, decoded) {
        if (global.INFO_LOG === true) { logger.info('[INFO] ' + MODULE_NAME + ' -> decodeToken -> onKeyReady -> onVerified -> Entering function.') }
