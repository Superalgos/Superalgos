                ...
                */
                ...
                for (let i = 0; i < SA.projects.network.globals.memory.arrays.P2P_NETWORK_NODES.length; i++) {
                    ...
                    */
                    ...
                    /* Check if we have enough Token Power allocated to the Task Server app to connect to this Network Node. Just warning as of now (introduction grace period). */
                    ...
                    if (nodeMinimumTokenAllocation > 0) {
                        ...
                        if (clientTokenAllocation === undefined || clientTokenAllocation < nodeMinimumTokenAllocation) {
                            SA.logger.info('PLEASE NOTE: Network Node ' + p2pNetworkNode.node.name + ' will soon require an allocation of min. ' + SA.projects.governance.utilities.balances.toSABalanceString(nodeMinimumTokenAllocation) + ' Token Power to the referenced Task Server App. Your current allocation is ' + SA.projects.governance.utilities.balances.toSABalanceString(clientTokenAllocation) + '.')
                            SA.logger.info('To ensure continued access to this node for the task you just started, allocate Token Power to "' + p2pNetworkClientIdentity.node.name + '" and contribute your updated profile.')
                            SA.logger.info('')
                        }
                    }
