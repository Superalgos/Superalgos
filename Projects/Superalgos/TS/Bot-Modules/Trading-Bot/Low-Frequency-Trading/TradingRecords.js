exports.newSuperalgosBotModulesTradingRecords = function (processIndex) {
    /*
            ...
            function persistRecords() {
                ...
                */
                if (product.config.saveAtCycle !== undefined) {
                    if (tradingEngine.tradingCurrent.episode.cycle.value !== product.config.saveAtCycle) {
                        return
                    }
                }

                /* Clean the file from information of previous executions */
                //pruneOutputFile(product, outputDatasetArray, tradingEngine.tradingCurrent.episode.candle.end.value)
                        ...
                        if (TS.projects.superalgos.globals.processVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).TRADING_PROCESSING_DAILY_FILES) {
                            if (product.config.doNotCutObjectInDays !== true) {
                                ...
                                */
                                ...
                                time frame. 
                                */
                                let currentDay = new Date(tradingEngine.tradingCurrent.episode.candle.end.value)
                                let nextDay = new Date(tradingEngine.tradingCurrent.episode.candle.end.value + 1)
