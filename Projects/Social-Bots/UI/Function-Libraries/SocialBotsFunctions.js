function newSocialBotsFunctionLibrarySocialBotsFunctions() {
    ...
    async function sendDiscordTestMessage(node) {
        // http api request to backend platform
        let endpoint = "Social-Bots/Discord-Test-Message"
        let token = UI.projects.visualScripting.utilities.nodeConfig.loadConfigProperty(node.payload, 'token')
        let channelId = UI.projects.visualScripting.utilities.nodeConfig.loadConfigProperty(node.payload, 'channelId')
    ...
    async function sendSlackTestMessage(node) {
        // http api request to backend platform
        let endpoint = "Social-Bots/Slack-Test-Message"
        let token = UI.projects.visualScripting.utilities.nodeConfig.loadConfigProperty(node.payload, 'token')
        let conversationId = UI.projects.visualScripting.utilities.nodeConfig.loadConfigProperty(node.payload, 'conversationId')
        let body = JSON.stringify({
            token: token,
