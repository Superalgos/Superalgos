    ...
    function finalize() {
        ...
        for (let i = 0; i < thisObject.connectors.length; i++) {
            ...
            for (let j = 0; j < connector.panels.length; j++) {
                UI.projects.superalgos.spaces.panelSpace.destroyPanel(connector.panels[j])
                    ...
                    function onPlotterInizialized() {
                        ...
                        if (productDefinition !== undefined) {
                            ...
                            for (let i = 0; i < plotterModule.panels.length; i++) {
                                ...
                                let plotterPanelHandle = UI.projects.superalgos.spaces.panelSpace.createNewPanel('Plotter Panel', parameters, owner, layer.session)
                                let plotterPanel = UI.projects.superalgos.spaces.panelSpace.getPanel(plotterPanelHandle)
