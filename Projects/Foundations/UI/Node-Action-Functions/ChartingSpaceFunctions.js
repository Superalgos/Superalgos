function newFoundationsFunctionLibraryChartingSpaceFunctions() {
                ...
                */
                ...
                for (let j = 0; j < allLayers.length; j++) {
                    ...
                    if (plotterModule === undefined) {
                        UI.projects.visualScripting.nodeActionFunctions.nodeDeleter.deleteUIObject(layer, rootNodes)
                    }
                }
                /*
    ...
    function createTimeMachine(dashboard, session, market, lanNetworkNode, rootNodes) {
        let mineProducts
        let timeMachine = UI.projects.visualScripting.nodeActionFunctions.uiObjectsFromNodes.addUIObject(dashboard, 'Time Machine')
        let exchange = market.payload.parentNode.payload.parentNode
        UI.projects.visualScripting.nodeActionFunctions.attachDetach.referenceAttachNode(timeMachine, session)
        ...
        for (let j = 0; j < mineProducts.length; j++) {
            ...
            */
            for (let k = 0; k < 3; k++) {
                let timelineChart = UI.projects.visualScripting.nodeActionFunctions.uiObjectsFromNodes.addUIObject(timeMachine, 'Timeline Chart')

                UI.projects.visualScripting.nodeActionFunctions.attachDetach.referenceAttachNode(timelineChart.layerManager, mineProduct)
                ...
                function deleteNodeByName(nodeName) {
                    ...
                    UI.projects.visualScripting.nodeActionFunctions.nodeDeleter.deleteUIObject(nodeToDelete, rootNodes)
                }
            }
        }
