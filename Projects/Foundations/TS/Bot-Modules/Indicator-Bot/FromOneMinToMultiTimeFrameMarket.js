exports.newFoundationsBotModulesFromOneMinToMultiTimeFrameMarket = function (
  processIndex
) {
  /*
    ...
    try {
        ...
        try {
          ...
          function loopBody() {
            const TIME_FRAME_LABEL =
              TS.projects.foundations.globals.timeFrames.marketTimeFramesArray()[
                timeFrameArrayIndex
              ][1];
      ...
      function buildOutput(allPreviousElements) {
        try {
          ...
          let lastDate =
            TS.projects.foundations.utilities.dateTimeFunctions.removeTime(
              new Date()
            );
          /*
            ...
            function loopBody() {
              ...
              if (
                ...
                for (let i = 0; i < previousElements.length; i++) {
                  ...
                  for (
                    let j = 0;
                    j <
                    node.outputDataset.referenceParent.parentNode.record
                      .properties.length;
                    j++
                  ) {
                    let property =
                  ...
                  if (
                    ...
                    TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                      ...
                      MODULE_NAME,
                      '[INFO] start -> buildOutput -> timeframesLoop -> loopBody -> Element # ' +
                        i +
                        ' @ ' +
                        TIME_FRAME_LABEL +
                        ' discarded for closing past the current process time.'
                    );
                  }
                }
              ...
              function aggregateAndWriteOutputFile(dependencyDailyFile) {
                ...
                function writeOutputFile(
                  ...
                  try {
                    let fileContent =
                      TS.projects.foundations.functionLibraries.fromOneMinToMultiTimeFrameFunctions.generateOutputFileContent(
                        node,
                        outputElementsCurrentTimeFrame
                      );
                    ...
                    let filePath =
                      ...
                      '/Output/' +
                      node.outputDataset.referenceParent.parentNode.config
                        .codeName +
                      '/' +
                      node.outputDataset.referenceParent.config.codeName +
                      '/' +
                    ...
                    TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                      ...
                      MODULE_NAME,
                      '[INFO] start -> writeOutputFile -> creating file at filePath = ' +
                        filePath
                    );
                    ...
                    function onFileCreated(err) {
                      if (
                        ...
                        TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                          ...
                          MODULE_NAME,
                          '[ERROR] start -> writeOutputFile -> onFileCreated -> err = ' +
                            err.stack
                        );
                        callBackFunction(err);
                        return;
                      ...
                      TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                        ...
                        '[WARN] start -> writeOutputFile -> onFileCreated ->  Finished with File @ ' +
                          TS.projects.foundations.globals.taskConstants
                            .TASK_NODE.parentNode.parentNode.parentNode
                            .referenceParent.baseAsset.referenceParent.config
                            .codeName +
                          '_' +
                          TS.projects.foundations.globals.taskConstants
                            .TASK_NODE.parentNode.parentNode.parentNode
                            .referenceParent.quotedAsset.referenceParent.config
                            .codeName +
                          ', ' +
                          ' filePath = ' +
                          filePath +
      ...
      callBackFunction(
        TS.projects.foundations.globals.standardResponses.DEFAULT_FAIL_RESPONSE
      );
    }
  }
};
