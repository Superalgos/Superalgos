exports.newFoundationsUtilitiesHttpResponses = function () {
    ...
    function respondWithImage(fileName, httpResponse) {
        ...
        function onFileRead(err, file) {
            if (err) {
                console.log((new Date()).toISOString(), '[ERROR] respondWithImage -> Image Not Found: ' + fileName + ' or Error = ' + err.stack)
                return
            }
    ...
    function respondWithFont(fileName, httpResponse) {
        ...
        function onFileRead(err, file) {
            if (err) {
                console.log((new Date()).toISOString(), '[ERROR] respondWithFont -> File Not Found: ' + fileName + ' or Error = ' + err.stack)
                return
            }
            httpResponse.setHeader('Cache-Control', 'no-cache, no-store, must-revalidate') // HTTP 1.1.
            httpResponse.setHeader('Pragma', 'no-cache') // HTTP 1.0.
    ...
    function respondWithStyleSheet(httpResponse, pathToApp, fileName) {
        ...
        function onFileRead(err, file) {
            try {
                ...
                console.log((new Date()).toISOString(), '[ERROR] respondWithStyleSheet -> File Not Found: ' + fileName + ' or Error = ' + err.stack)
            }
        }
    }
    ...
    function respondWithProjectFolderFileList(httpResponse, projectFolderName, rootObjectName) {
        {
            ...
            for (let i = 0; i < projects.length; i++) {
                ...
                try {
                    ...
                    console.log((new Date()).toISOString(), '[ERROR] returnProjectFolderFileList -> Error reading a directory content. filePath = ' + dirPath)
                    console.log((new Date()).toISOString(), '[ERROR] returnProjectFolderFileList -> err.stack = ' + err.stack)
