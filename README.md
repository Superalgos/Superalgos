...
Download the software package from the [latest software release](https://github.com/Superalgos/DesktopApp/releases/tag/v.0.0.1-alpha) (```Superalgos-Desktop-App.zip```) and extract the file in an accessible route (you will launch the app from these folders).

Download historic market data from the [latest data release](https://github.com/Superalgos/DesktopApp/releases/tag/data.poloniex.btc.usdt.2019.08.27) (```Poloniex.BTC.USDT.Historic.Data.zip```) and extract the file inside the ```Superalgos Desktop App``` folder generated by the software package ZIP.

You should end up with the following folder structure:

![image](https://user-images.githubusercontent.com/13994516/63965100-be6a7b80-ca98-11e9-9398-40b46be9ecf5.png)

> Make sure the data ended up as the ```Data-Storage``` folder, right inside ```Superalgos Desktop App```

**2. Continue with [Running the App](#running-the-app)**

> NOTE FOR DEVELOPERS: Instead of using the EXE file to run the app, you may want to use ```node server.js``` from within the ```WebServer``` folder to run on your full Node Js installation.

## The Long Version for Regular Folks on Windows and Mac

### Downloads

**1. Download the Software Package**:

Go to the [latest Superalgos Desktop App release](https://github.com/Superalgos/DesktopApp/releases/tag/v.0.0.1-alpha) and download ```Superalgos-Desktop-App.zip```.

**2. Extract the ZIP file**: 

How you extract/unpack the files depends on the software you use to handle ZIP files. Double clicking the ZIP file should work in most scenarios, however, some systems may "open" the file instead of "extracting" it. Make sure you are "extracting" and not just "opening" the file. You may also right-click the file and check the options available on the menu.
...
Go to the [latest Data release](https://github.com/Superalgos/DesktopApp/releases/tag/data.poloniex.btc.usdt.2019.08.27) and download ```Poloniex.BTC.USDT.Historic.Data.zip```.

The ZIP file includes highly-compressed complete historic BTC/USDT market data at Poloniex. Expect decompression to take from 30 to 60 minutes, depending on your system. Once you have the historic data, the app will make sure the data stays up to date.

**4. Extract the ZIP file**:

You need to extract the file inside the ```Superalgos Desktop App``` folder generated by the first ZIP file. You can do this either moving ```Poloniex.BTC.USDT.Historic.Data.zip``` inside the ```Superalgos Desktop App``` folder and selecting ```Extract here``` from the menu poping up upon right-clicking the file, or by extracting anywhere else and the moving the resulting ```Data-Storage``` folder inside the ```Superalgos Desktop App``` folder.
...
Drag one of the sample _workspaces_ in the ```Superalgos Desktop App/Quick-Start-Examples``` folder and drop it on the _Designer_ (in the bottom half of the screen, the area with a black background) to start with a working template.
