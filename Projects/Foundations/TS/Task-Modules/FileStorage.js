exports.newFileStorage = function newFileStorage(processIndex, host, port) {
  ...
  if (
    TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
      processIndex
    ).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT === undefined
  ) {
    // Dummy logger
    logger = {};
    function write() {}
    ...
    logger =
      TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
        processIndex
      ).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT;
  }
  return thisObject;
  ...
  async function asyncCreateTextFile(
    filePath,
    fileContent,
    keepPrevious,
    noTemp
  ...
  function getTextFile(filePath, callBackFunction, noRetry, canUsePrevious) {
    ...
    function recursiveGetTextFile(
      ...
      try {
        logger.write(
          MODULE_NAME,
          '[INFO] FileStorage -> getTextFile -> fileLocation: ' + fileLocation
        );
        ...
        function onFileRead(err, text) {
          ...
          if (err) {
            if (err.code === 'ENOENT') {
              // since files are deleted before being replaced, it can happen that it does not exist and after a retry it does.
              if (noRetry === true) {
                logger.write(
                  MODULE_NAME,
                  '[WARN] FileStorage -> getTextFile -> onFileRead -> File does not exist. Not Retrying. '
                );
                let customResponse = {
                  result:
                    TS.projects.foundations.globals.standardResponses
                      .CUSTOM_FAIL_RESPONSE.result,
              ...
              logger.write(
                MODULE_NAME,
                '[WARN] FileStorage -> getTextFile -> onFileRead -> File does not exist. Retrying. '
              );
            ...
            logger.write(
              MODULE_NAME,
              '[WARN] FileStorage -> getTextFile -> onFileRead -> Error reading file -> err = ' +
                err.stack
            );
            logger.write(
              MODULE_NAME,
              '[WARN] FileStorage -> getTextFile -> onFileRead -> Error reading file -> filePath = ' +
                filePath
            );
            logger.write(
              MODULE_NAME,
          ...
          if (text.toString() === '' && canUsePrevious !== true) {
            logger.write(
              MODULE_NAME,
              '[WARN] FileStorage -> getTextFile -> onFileRead -> Read and Empty File -> filePath = ' +
                filePath
            );
            setTimeout(retry, retryTimeToUse);
            return;
          ...
          try {
            ...
            if (canUsePrevious === true) {
              logger.write(
                MODULE_NAME,
                '[WARN] FileStorage -> getTextFile -> onFileRead -> Could read the file, but could not parse it as it is not a valid JSON. Will try to read the PREVIOUS version instead. -> file = ' +
                  fileLocation
              );
              ...
              function onPreviousFileRead(err, text) {
                if (err) {
                  logger.write(
                    MODULE_NAME,
                    '[WARN] FileStorage -> getTextFile -> onFileRead -> canUsePrevious -> Could not read the Previous file either. Giving up.'
                  );
                  logger.write(
                    MODULE_NAME,
                    '[WARN] FileStorage -> getTextFile -> onFileRead -> canUsePrevious -> err = ' +
                      err.stack
                  );
                  callBackFunction(
                    TS.projects.foundations.globals.standardResponses
              ...
              logger.write(
                MODULE_NAME,
                '[WARN] FileStorage -> getTextFile -> onFileRead -> Could read the file, but could not parse it as it is not a valid JSON.'
              );
              logger.write(
                MODULE_NAME,
                '[WARN] FileStorage -> getTextFile -> onFileRead -> err = ' +
                  err.stack
              );
              logger.write(
                MODULE_NAME,
                '[WARN] FileStorage -> getTextFile -> onFileRead -> text.toString() = ' +
                  text.toString()
              );
              setTimeout(retry, retryTimeToUse);
              return;
        ...
        logger.write(
          MODULE_NAME,
          '[WARN] FileStorage -> getTextFile -> Error reading file -> file = ' +
            fileLocation
        );
        logger.write(
          MODULE_NAME,
          '[WARN] FileStorage -> getTextFile -> Error reading file -> err = ' +
            err.stack
        );
        retry();
      }
      ...
      function retry() {
        if (currentRetryGetTextFile < MAX_RETRY) {
          ...
          logger.write(
            MODULE_NAME,
            '[INFO] FileStorage -> getTextFile -> retry -> Will try to read the file again -> Retry #: ' +
              currentRetryGetTextFile
          );
          recursiveGetTextFile(
            filePath,
            callBackFunction,
            noRetry,
            canUsePrevious
          ...
          if (fileDoesNotExist === true) {
            logger.write(
              MODULE_NAME,
              '[WARN] FileStorage -> getTextFile -> retry -> Max retries reached reading a file. File Not Found.'
            );
            let customResponse = {
              result:
                TS.projects.foundations.globals.standardResponses
            ...
            logger.write(
              MODULE_NAME,
              '[ERROR] FileStorage -> getTextFile -> retry -> Max retries reached reading a file. Giving up.'
            );
            logger.write(
              MODULE_NAME,
              '[ERROR] FileStorage -> getTextFile -> retry -> file = ' +
                fileLocation
            );
            callBackFunction(
              TS.projects.foundations.globals.standardResponses
  ...
  function createTextFile(
    ...
    function recursiveCreateTextFile(
      filePath,
      fileContent,
      callBackFunction,
      keepPrevious,
      ...
      try {
        logger.write(
          MODULE_NAME,
          '[INFO] FileStorage -> createTextFile -> fileLocation: ' +
            fileLocation
        );

        /* If necesary a folder or folders are created before writing the file to disk. */
        SA.projects.foundations.utilities.filesAndDirectories.mkDirByPathSync(
          fileLocation
        );
        ...
        function onFileWriten(err) {
          ...
          if (err) {
            logger.write(
              MODULE_NAME,
              '[WARN] FileStorage -> createTextFile -> onFileWriten -> Error writing file -> file = ' +
                fileLocation +
                '.tmp'
            );
            logger.write(
              MODULE_NAME,
              '[WARN] FileStorage -> createTextFile -> onFileWriten -> Error writing file -> err = ' +
                err.stack
            );
            setTimeout(retry, retryTimeToUse);
          } else {
            ...
            if (keepPrevious === true) {
              ...
              function onUnlinked(err) {
                ...
                if (code !== '' && code !== 'ENOENT') {
                  logger.write(
                    MODULE_NAME,
                    '[WARN] FileStorage -> createTextFile -> onFileWriten -> onUnlinked -> Error deleting file -> file = ' +
                      fileLocation +
                      '.Previous.json'
                  );
                  logger.write(
                    MODULE_NAME,
                    '[WARN] FileStorage -> createTextFile -> onFileWriten -> onUnlinked -> Error deleting file -> err = ' +
                      err.stack
                  );
                  setTimeout(retry, retryTimeToUse);
                  return;
                ...
                function onOriginalRenamed(err) {
                  ...
                  if (code !== '' && code !== 'ENOENT') {
                    // Unless the file does not exists we do this...
                    logger.write(
                      MODULE_NAME,
                      '[WARN] FileStorage -> createTextFile -> onFileWriten -> onUnlinked -> onOriginalRenamed -> Error renaming original file -> file = ' +
                        fileLocation
                    );
                    logger.write(
                      MODULE_NAME,
                      '[WARN] FileStorage -> createTextFile -> onFileWriten -> onUnlinked -> onOriginalRenamed -> Error renaming original file -> err = ' +
                        err.stack
                    );
                    setTimeout(retry, retryTimeToUse);
                    return;
                  ...
                  function onTempRenamed(err) {
                    if (err) {
                      logger.write(
                        MODULE_NAME,
                        '[WARN] FileStorage -> createTextFile -> onFileWriten -> onUnlinked -> onOriginalRenamed -> onTempRenamed -> Error renaming temp file -> file = ' +
                          fileLocation +
                          '.tmp'
                      );
                      logger.write(
                        MODULE_NAME,
                        '[WARN] FileStorage -> createTextFile -> onFileWriten -> onUnlinked -> onOriginalRenamed -> onTempRenamed -> Error renaming temp file -> err = ' +
                          err.stack
                      );
                      setTimeout(retry, retryTimeToUse);
                    } else {
        ...
        logger.write(
          MODULE_NAME,
          '[WARN] FileStorage -> createTextFile -> Error writing file -> file = ' +
            fileLocation
        );
        logger.write(
          MODULE_NAME,
          '[WARN] FileStorage -> createTextFile -> Error writing file -> err = ' +
            err.stack
        );
        retry();
      }
      ...
      function retry() {
        if (currentRetryWriteTextFile < MAX_RETRY) {
          ...
          logger.write(
            MODULE_NAME,
            '[INFO] FileStorage -> createTextFile -> retry -> Will try to write the file again -> Retry #: ' +
              currentRetryWriteTextFile
          );
          recursiveCreateTextFile(
            filePath,
          ...
          logger.write(
            MODULE_NAME,
            '[ERROR] FileStorage -> createTextFile -> retry -> Max retries reached writing a file. Giving up.'
          );
          logger.write(
            MODULE_NAME,
            '[ERROR] FileStorage -> createTextFile -> retry -> file = ' +
              fileLocation
          );
          callBackFunction(
            TS.projects.foundations.globals.standardResponses
  ...
  function deleteTextFile(filePath, callBackFunction) {
    ...
    function recursiveDeleteTextFile(filePath, callBackFunction) {
      logger.write(
        MODULE_NAME,
        '[INFO] FileStorage -> deleteTextFile -> Entering Function.'
      );
      ...
      try {
        logger.write(
          MODULE_NAME,
          '[INFO] FileStorage -> deleteTextFile -> fileLocation: ' +
            fileLocation
        );
        ...
        logger.write(
          MODULE_NAME,
          '[WARN] FileStorage -> deleteTextFile -> Error writing file -> file = ' +
            fileLocation
        );
        logger.write(
          MODULE_NAME,
          '[WARN] FileStorage -> deleteTextFile -> Error writing file -> err = ' +
            err.stack
        );
        if (callBackFunction !== undefined) {
          callBackFunction(
      ...
      function retry() {
        if (currentRetryDeleteTextFile < MAX_RETRY) {
          ...
          logger.write(
            MODULE_NAME,
            '[INFO] FileStorage -> deleteTextFile -> retry -> Will try to delete the file again -> Retry #: ' +
              currentRetryDeleteTextFile
          );
          recursiveDeleteTextFile(filePath, callBackFunction);
        } else {
          ...
          logger.write(
            MODULE_NAME,
            '[ERROR] FileStorage -> deleteTextFile -> retry -> Max retries reached deleting a file. Giving up.'
          );
          logger.write(
            MODULE_NAME,
            '[ERROR] FileStorage -> deleteTextFile -> retry -> file = ' +
              fileLocation
          );
          if (callBackFunction !== undefined) {
            callBackFunction(
  ...
  function getFileViaHTTP(filePath, callback, callBackFunction) {
    try {
      ...
      let url = 'http://' + host + ':' + port + '/Storage/' + filePath;

      logger.write(
        MODULE_NAME,
        '[INFO] FileStorage -> getFileViaHTTP -> url = ' + url
      );
      ...
      request.on('error', function (err) {
        logger.write(
          MODULE_NAME,
          '[ERROR] getFileViaHTTP -> onError -> err = ' + err.stack
        );
        logger.write(
          MODULE_NAME,
          '[ERROR] getFileViaHTTP -> onError -> Failed to fetch file via HTTP. Will retry later. '
        );
        callBackFunction(
          TS.projects.foundations.globals.standardResponses
      ...
      function onResponse(response) {
        try {
          ...
          logger.write(
            MODULE_NAME,
            '[ERROR] getFileViaHTTP -> onResponse -> err = ' + err.stack
          );
          logger.write(
            MODULE_NAME,
            '[ERROR] getFileViaHTTP -> onResponse -> Failed to fetch file via HTTP. Will retry later. '
          );
          callBackFunction(
            TS.projects.foundations.globals.standardResponses
              .DEFAULT_RETRY_RESPONSE
      ...
      logger.write(
        MODULE_NAME,
        '[ERROR] getFileViaHTTP -> Failed to fetch file via HTTP. Will retry later. '
      );
      callBackFunction(
        TS.projects.foundations.globals.standardResponses.DEFAULT_RETRY_RESPONSE
      );
    }
  }
};
