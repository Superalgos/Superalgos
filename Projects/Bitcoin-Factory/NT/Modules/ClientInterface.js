    ...
    */
    ...
    async function messageReceived(
        ...
        async function testClientMessage(queryReceived, userProfile) {
            ...
            console.log((new Date()).toISOString(), '[WARN] Request From Test Client v.' + testClientVersion + '       -> timestamp = ' + (new Date()).toISOString(requestToServer.timestamp) + ' -> Websockets Clients = ' + SA.projects.foundations.utilities.miscellaneousFunctions.pad(Number(connectedUserProfilesLabel), 3) + ' -> Clients Requests Queue Size = ' + SA.projects.foundations.utilities.miscellaneousFunctions.pad(requestsToServer.length, 3) + ' -> userProfile = ' + userProfile)
        ...
        async function forecastClientMessage(queryReceived, userProfile) {
            ...
            requestsToServer.push(requestToServer)
            console.log((new Date()).toISOString(), '[WARN] Request From Forecast Client       -> timestamp = ' + (new Date()).toISOString(requestToServer.timestamp) + ' -> Websockets Clients = ' + SA.projects.foundations.utilities.miscellaneousFunctions.pad(Number(connectedUserProfilesLabel), 3) + ' -> Clients Requests Queue Size = ' + SA.projects.foundations.utilities.miscellaneousFunctions.pad(requestsToServer.length, 3) + ' -> userProfile = ' + userProfile)
            ...
            async function promiseWork(resolve, reject) {
                if (requestsToServer.length === 0) {
                    ...
                    if (now - requestToServer.timestamp < 2 * 60 * 1000) {
                        ...
                        console.log((new Date()).toISOString(), '[WARN] Request Sent to Server             -> timestamp = ' + (new Date()).toISOString(requestToServer.timestamp) + ' -> Websockets Clients = ' + SA.projects.foundations.utilities.miscellaneousFunctions.pad(Number(connectedUserProfilesLabel), 3) + ' -> Clients Requests Queue Size = ' + SA.projects.foundations.utilities.miscellaneousFunctions.pad(requestsToServer.length, 3) + ' -> userProfile = ' + userProfile)
                        resolve(response)
                    } else {
                        console.log((new Date()).toISOString(), '[WARN] Request Expired                    -> timestamp = ' + (new Date()).toISOString(requestToServer.timestamp) + ' -> Websockets Clients = ' + SA.projects.foundations.utilities.miscellaneousFunctions.pad(Number(connectedUserProfilesLabel), 3) + ' -> Clients Requests Queue Size = ' + SA.projects.foundations.utilities.miscellaneousFunctions.pad(requestsToServer.length, 3) + ' -> userProfile = ' + userProfile + ' -> requestToServer.queryReceived = ' + JSON.stringify(requestToServer.queryReceived))
                        let response = {
                            result: 'Ok',
