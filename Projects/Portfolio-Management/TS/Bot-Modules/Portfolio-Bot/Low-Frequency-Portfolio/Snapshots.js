exports.newPortfolioManagementBotModulesSnapshots = function(processIndex) {
    /*
    ...
    function initialize() {
        ...
        if (portfolioEngine.portfolioCurrent.portfolioEpisode.portfolioEpisodeCounters.positions.value > 1) {
    ...
    function finalize() {
        ...
        if (portfolioEngine.portfolioCurrent.strategy.begin.value > 0) {
            strategyExit()
            positionExit()
        }
        writeSnapshotFiles()
    ...
    function strategyEntry() {
        /* 
            We will record here a snapshot of all values used in conditions and formulas within the Portfolio System. 
    ...
    function strategyExit() {
        getResults(portfolioEngine.portfolioCurrent.strategy.begin.value, portfolioEngine.portfolioCurrent.strategy.end.value)
    ...
    function positionExit() {
        getResults(portfolioEngine.portfolioCurrent.position.begin.value, portfolioEngine.portfolioCurrent.position.end.value)
                    ...
                    function addValues(keys, values) {
                        ...
                        if (existingKey === undefined) { // means that at the current loop this property of this product was not used before.
                            ...
                            try {
                                ...
                                portfolioSystem.addError([nodeWithCode.id, message, docs])
                                values.push(0)
                            }
                        }
                    }
    ...
    function getResults(openDatetime, closeDatetime) {
        ...
        let baseOpen = portfolioEngine.portfolioCurrent.strategyCloseStage.stageBaseAsset.sizeFilled.value - portfolioEngine.portfolioCurrent.strategyCloseStage.stageBaseAsset.feesPaid.value
        let baseClose = portfolioEngine.portfolioCurrent.strategyOpenStage.stageBaseAsset.sizeFilled.value - portfolioEngine.portfolioCurrent.strategyOpenStage.stageBaseAsset.feesPaid.value
        let quoteOpen = portfolioEngine.portfolioCurrent.strategyOpenStage.stageQuotedAsset.sizeFilled.value - portfolioEngine.portfolioCurrent.strategyOpenStage.stageQuotedAsset.feesPaid.value
        let quoteClose = portfolioEngine.portfolioCurrent.strategyCloseStage.stageQuotedAsset.sizeFilled.value - portfolioEngine.portfolioCurrent.strategyCloseStage.stageQuotedAsset.feesPaid.value
        ...
        closeValues = [
            portfolioEngine.portfolioCurrent.portfolioEpisode.portfolioEpisodeCounters.positions.value, // Position Number
            (new Date(openDatetime)).toISOString(), // Open Datetime
            (new Date(closeDatetime)).toISOString(), // Open Datetime
            portfolioEngine.portfolioCurrent.strategy.strategyName.value, // Strategy Name
            portfolioEngine.portfolioCurrent.strategy.situationName.value, // Trigger On Situation
            portfolioEngine.portfolioCurrent.position.situationName.value, // Take Position Situation
            ...
            portfolioEngine.portfolioCurrent.position.positionBaseAsset.hitFail.value, // Result in Base Asset
            portfolioEngine.portfolioCurrent.position.positionQuotedAsset.hitFail.value, // Result in Base Asset
            portfolioEngine.portfolioCurrent.position.positionBaseAsset.ROI.value, // ROI in Base Asset <- not sure this works as intended
            baseProfit + '%', // % profit / loss in base Asset
            baseProfit === 0 ? 'Even' : (baseProfit > 0 ? "Profit" : "Loss"),

            portfolioEngine.portfolioCurrent.position.positionQuotedAsset.ROI.value, // ROI in Quoted Asset <- not sure this works as intended
            qouteProfit + '%', // % profit loss in quoted Asset
            qouteProfit === 0 ? 'Even' : (qouteProfit > 0 ? "Profit" : "Loss"),
            portfolioEngine.portfolioCurrent.position.exitType.value, // Exit Type
            portfolioEngine.portfolioCurrent.position.positionCounters.periods.value // periods counter
        ]
