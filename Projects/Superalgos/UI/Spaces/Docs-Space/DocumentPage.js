    ...
    function render() {
        ...
        function buildHtmlPage() {
            ...
            if (UI.projects.superalgos.spaces.docsSpace.currentDocumentBeingRendered.category === 'Topic') {
                ...
                HTML = HTML = HTML + '<div class="docs-topic-index">' // Container for whole Topic Navigation including buttons, title and index
                ...
                HTML = HTML + '<h3 class="docs-h3"> Table of Contents</h3>'

                generateTopicMultiPageIndex()

                HTML = HTML = HTML + '</div>'  // END Container for whole Topic Navigation
            }
            ...
            if (UI.projects.superalgos.spaces.docsSpace.currentDocumentBeingRendered.category === 'Tutorial') {
                ...
                /* Tutorial Title */
                titleLabel = thisObject.docsSchemaDocument.tutorial + ' Tutorial Content'
                HTML = HTML + '<h2 id="' + UI.projects.superalgos.spaces.docsSpace.currentDocumentBeingRendered.type.toLowerCase().replace(' ', '-') + '" > ' + titleLabel + '</h2>'
            ...
            function generateTopicPreviousAndNextPageNavigation() {
                ...
                for (let i = 0; i < orderedTopicPageIndexArray.length; i++) {
                    ...
                    if (thisObject.docsSchemaDocument.type === arrayItem.type) {
                        ...
                        HTML = HTML + '<p>You just read page <strong>' + thisObject.docsSchemaDocument.pageNumber + '</strong> in the series. Click the buttons below for the previous and next pages...</p>'

                        HTML = HTML + '<div class="docs-topic-navigation"><div>'
                        if (previousPage !== undefined) {
                            HTML = HTML + '<span" onClick="UI.projects.superalgos.spaces.docsSpace.navigateTo(\'' + UI.projects.superalgos.spaces.docsSpace.currentDocumentBeingRendered.project + '\', \'' + 'Topic' + '\', \'' + previousPage.type.replace(/'/g, 'AMPERSAND') + '\')"><button>' + previousPage.type + '</button></span><br/>&#8592;'
                        }
                        HTML = HTML + '</div><div>'
                        if (nextPage !== undefined) {
                            HTML = HTML + '<span" onClick="UI.projects.superalgos.spaces.docsSpace.navigateTo(\'' + UI.projects.superalgos.spaces.docsSpace.currentDocumentBeingRendered.project + '\', \'' + 'Topic' + '\', \'' + nextPage.type.replace(/'/g, 'AMPERSAND') + '\')"><button>' + nextPage.type + '</button></span><br/>&#8594;'
                        }
                        HTML = HTML + '</div></div>'
                        return
                    }
            ...
            function generateTutorialPreviousAndNextPageNavigation() {
                ...
                for (let i = 0; i < orderedTutorialPageIndexArray.length; i++) {
                    ...
                    if (thisObject.docsSchemaDocument.type === arrayItem.type) {
                        ...
                        HTML = HTML + '<div class="docs-tutorial-navigation"><div>'
                        if (previousPage !== undefined) {
                            HTML = HTML + '<span" onClick="UI.projects.superalgos.spaces.docsSpace.navigateTo(\'' + UI.projects.superalgos.spaces.docsSpace.currentDocumentBeingRendered.project + '\', \'' + 'Tutorial' + '\', \'' + previousPage.type.replace(/'/g, 'AMPERSAND') + '\')"><button>' + previousPage.type + '</button></span>'
                        }
                        HTML = HTML + '</div><div> << Current Page: ' + thisObject.docsSchemaDocument.pageNumber + ' >> </div><div>'
                        if (nextPage !== undefined) {
                            HTML = HTML + '<span" onClick="UI.projects.superalgos.spaces.docsSpace.navigateTo(\'' + UI.projects.superalgos.spaces.docsSpace.currentDocumentBeingRendered.project + '\', \'' + 'Tutorial' + '\', \'' + nextPage.type.replace(/'/g, 'AMPERSAND') + '\')"><button>' + nextPage.type + '</button></span>'
                        }
                        HTML = HTML + '</div></div>'
                        return
                    }
            ...
            function generateTutorialMultiPageIndex() {
                ...
                */
                ...
                for (let i = 0; i < orderedTutorialPageIndexArray.length; i++) {
                    ...
                    autoGeneratedParagraphIndex++
                    HTML = HTML + '<p>' + arrayItem.pageNumber + '. ' + '<a class="docs-link" onClick="UI.projects.superalgos.spaces.docsSpace.navigateTo(\'' + UI.projects.superalgos.spaces.docsSpace.currentDocumentBeingRendered.project + '\', \'' + 'Tutorial' + '\', \'' + arrayItem.type.replace(/'/g, 'AMPERSAND') + '\')">' + paragraph.text + '</a></p>'
                }
            }
