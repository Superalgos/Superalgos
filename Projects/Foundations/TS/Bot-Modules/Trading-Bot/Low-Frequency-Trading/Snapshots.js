exports.newFoundationsBotModulesSnapshots = function(processIndex) {
    ...
    function initialize() {
        ...
        if (
            tradingEngine.tradingCurrent.tradingEpisode.tradingEpisodeCounters
            .positions.value > 1
        ) {
    ...
    function strategyExit() {
        getResults(
            tradingEngine.tradingCurrent.strategy.begin.value,
            tradingEngine.tradingCurrent.strategy.end.value
        );
    ...
    function positionExit() {
        getResults(
            tradingEngine.tradingCurrent.position.begin.value,
            tradingEngine.tradingCurrent.position.end.value
        );
    ...
    function addCodeToSnapshot(nodeWithCode) {
        if (nodeWithCode === undefined) {
            return;
        }
        if (nodeWithCode.code === undefined) {
            return;
        }
        ...
        try {
            ...
            for (let i = 0; i < instructionsArray.length; i++) {
                ...
                if (instruction.indexOf('chart') >= 0) {
                    ...
                    if (timeFrame !== 'atAnyTimeFrame') {
                        timeFrame =
                            timeFrame.substring(2, 4) + '-' + timeFrame.substring(4, 7);
                    }
                    ...
                    function addValues(keys, values) {
                        ...
                        if (existingKey === undefined) {
                            // means that at the current loop this property of this product was not used before.
                            keys.set(key, key);
                            if (
                                createHeaders === true
                            ) {
            ...
            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                ...
                MODULE_NAME,
                '[ERROR] runSimulation -> addCodeToSnapshot -> nodeWithCode.code = ' +
                nodeWithCode.code
            );
            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                processIndex
                ...
                MODULE_NAME,
                '[ERROR] runSimulation -> addCodeToSnapshot -> err = ' + err.stack
            );
        }
    }
    ...
    function getResults(openDatetime, closeDatetime) {
        ...
        closeValues = [
            tradingEngine.tradingCurrent.tradingEpisode.tradingEpisodeCounters
            .positions.value, // Position Number
        ...
        if (
            createCloseHeaders === true
        ) {
            snapshots.headers = closeHeaders.concat(
                JSON.parse(JSON.stringify(snapshots.headers))
            );
            createCloseHeaders = false;
        }
    }
    ...
    function writeSnapshotFile(snapshotArray, pFileName) {
        try {
            let fileStorage =
                TS.projects.foundations.taskModules.fileStorage.newFileStorage(
                    processIndex
                );
            ...
            function writeOutput(fileContent) {
                ...
                function onFileCreated(err) {
                    if (
                        err.result !==
                        TS.projects.foundations.globals.standardResponses.DEFAULT_OK_RESPONSE
                        .result
                    ) {
                        TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                            processIndex
                            ...
                            MODULE_NAME,
                            '[ERROR] writeSnapshotFile -> onFileCreated -> err = ' + err.stack
                        );
                        TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                            processIndex
                        ).BOT_MAIN_LOOP_LOGGER_MODULE_OBJECT.write(
                            MODULE_NAME,
                            '[ERROR] writeSnapshotFile -> onFileCreated -> filePath = ' +
                            filePath
                        );
                        TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                            processIndex
                            ...
                            MODULE_NAME,
                            '[ERROR] writeSnapshotFile -> onFileCreated -> market = ' +
                            ...
                            '_' +
                            TS.projects.foundations.globals.taskConstants.TASK_NODE.parentNode
                            .parentNode.parentNode.referenceParent.quotedAsset
                    ...
                    if (
                        ...
                        try {
                            ...
                            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                                ...
                                MODULE_NAME,
                                '[ERROR] start -> loadExistingFiles -> loopBody -> readExistingFile -> onFileReceived -> fileName = ' +
                                filePath
                            );
                            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                                processIndex
                                ...
                                MODULE_NAME,
                                '[ERROR] start -> loadExistingFiles -> loopBody -> readExistingFile -> onFileReceived -> filePath = ' +
                                filePath
                            );
                            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                                processIndex
                                ...
                                MODULE_NAME,
                                '[ERROR] start -> loadExistingFiles -> loopBody -> readExistingFile -> onFileReceived -> err = ' +
                                err.stack
                            );
                            TS.projects.foundations.globals.loggerVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(
                                processIndex
