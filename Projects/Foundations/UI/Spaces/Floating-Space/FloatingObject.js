...
function newFloatingObject() {
    ...
    function finalize() {
        ...
        UI.projects.foundations.spaces.floatingSpace.container.eventHandler.stopListening(spaceMouseOverEventSubscriptionId)
    ...
    function initialize(type, payload) {
        ...
        spaceMouseOverEventSubscriptionId = UI.projects.foundations.spaces.floatingSpace.container.eventHandler.listenToEvent('onMouseOver', mouseOverFlotingSpace)
        spaceFocusAquiredEventSubscriptionId = UI.projects.foundations.spaces.floatingSpace.container.eventHandler.listenToEvent('onFocusAquired', someoneAquiredFocus)
    ...
    function physics() {
        ...
        if (UI.projects.foundations.spaces.floatingSpace.isItFar(thisObject.payload)) { return }
        thisObjectPhysics()
        if (thisObject.payload.uiObject !== undefined) {
    ...
    function syncStylePhysics() {
        if (thisObject.rawFontSize !== UI.projects.foundations.spaces.floatingSpace.style.node.fontSize) {
            thisObject.rawFontSize = UI.projects.foundations.spaces.floatingSpace.style.node.fontSize
            thisObject.targetFontSize = thisObject.rawFontSize * 1
        }
        if (thisObject.rawImageSize !== UI.projects.foundations.spaces.floatingSpace.style.node.imageSize) {
            thisObject.rawImageSize = UI.projects.foundations.spaces.floatingSpace.style.node.imageSize
            thisObject.targetImageSize = thisObject.rawImageSize * 1
        }
        thisObject.typeStrokeStyle = 'rgba(' + UI.projects.foundations.spaces.floatingSpace.style.node.type.fontColor + ', 1)'
        thisObject.nameStrokeStyle = 'rgba(' + UI.projects.foundations.spaces.floatingSpace.style.node.name.fontColor + ', 1)'
    }
    ...
    function positionContraintsPhysics() {
        ...
        const DEFAULT_NODE_TO_NODE_DISTANCE = 500 * UI.projects.foundations.spaces.floatingSpace.settings.node.distancePercentage / 100
        ...
        if (thisObject.angleToParent !== ANGLE_TO_PARENT.NOT_FIXED && thisObject.isOnFocus !== true) {
            ...
            if (parent === undefined) {
                ...
                parentChildren = UI.projects.foundations.functionLibraries.nodeChildren.childrenCount(parent, thisObject.payload.node)
    ...
    function thisObjectPhysics() {
        ...
        const ANIMATION_STEPS = UI.projects.foundations.spaces.floatingSpace.settings.node.animationSteps
    ...
    function targetRadiusWithFocus() {
        return thisObject.rawRadius * ON_FOCUS_RADIUS_FACTOR * UI.projects.foundations.spaces.floatingSpace.settings.node.radiusPercentage / 100
    }

    function targetRadiusWithoutFocus() {
        return thisObject.rawRadius * 1 * UI.projects.foundations.spaces.floatingSpace.settings.node.radiusPercentage / 100
    }

    function setFocus(point) {
        thisObject.targetRadius = targetRadiusWithFocus()
        thisObject.currentMass = thisObject.rawMass * UI.projects.foundations.spaces.floatingSpace.settings.node.massPercentage / 100
        ...
        UI.projects.foundations.spaces.floatingSpace.container.eventHandler.raiseEvent('onFocusAquired', thisObject)
    ...
    function drawBackground() {
        ...
        if (UI.projects.foundations.spaces.floatingSpace.isItFar(thisObject.payload) && thisObject.payload.uiObject.isShowing === false) { return }
    ...
    function initializeRadius(suggestedValue) {
        ...
        thisObject.targetRadius = radius * UI.projects.foundations.spaces.floatingSpace.settings.node.radiusPercentage / 100
