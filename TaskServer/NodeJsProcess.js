exports.newNodeJsProcess = function () {
    ...
    function initialize() {
        ...
        process.on('uncaughtException', function (err) {
            console.log((new Date()).toISOString(), '[ERROR] Task Server -> Node JS Process -> uncaughtException -> err.message = ' + err.message)
            console.log((new Date()).toISOString(), '[ERROR] Task Server -> Node JS Process -> uncaughtException -> err.stack = ' + err.stack)
            if (err !== undefined) { console.log(err.stack) }
            TS.projects.foundations.functionLibraries.nodeJSFunctions.exitProcess()
        })

        process.on('unhandledRejection', (reason, p) => {
            console.log((new Date()).toISOString(), '[ERROR] Task Server -> Node JS Process -> unhandledRejection -> reason = ' + JSON.stringify(reason))
            console.log((new Date()).toISOString(), '[ERROR] Task Server -> Node JS Process -> unhandledRejection -> p = ' + JSON.stringify(p))
        ...
        process.on('exit', function (config) {
            ...
            //console.log((new Date()).toISOString(), '[INFO] Task Server -> Node JS Process -> process.on.exit -> About to exit -> config = ' + config)
        })
        ...
        process.on('message', message => {
            if (message === 'Stop this Task') {
                ...
                */
                ...
                console.log((new Date()).toISOString(), '[INFO] Task Server -> Node JS Process -> process.on -> Stopping Task ' + key + '. Nodejs process will be exited in less than 1 minute.')
