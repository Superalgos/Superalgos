            ...
            function persistRecords() {
                ...
                */
                if (product.config.saveAtCycle !== undefined) {
                    if (learningEngine.learningCurrent.episode.cycle.value !== product.config.saveAtCycle) {
                        return
                    }
                }

                /* Clean the file from information of previous executions */
                //pruneOutputFile(product, outputDatasetArray, learningEngine.learningCurrent.episode.candle.end.value)
                        ...
                        if (TS.projects.superalgos.globals.processVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).TRADING_PROCESSING_DAILY_FILES) {
                            if (product.config.doNotCutObjectInDays !== true) {
                                ...
                                */
                                ...
                                time frame. 
                                */
                                let currentDay = new Date(learningEngine.learningCurrent.episode.candle.end.value)
                                let nextDay = new Date(learningEngine.learningCurrent.episode.candle.end.value + 1)
