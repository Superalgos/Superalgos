function newRateScale() {
    ...
    function draggingPhysics() {
        ...
        let mouseNoZoom = UI.projects.foundations.spaces.chartingSpace.viewport.unTransformThisPoint(canvas.mouse.position)
        ...
        let newMaxRate = UI.projects.foundations.utilities.dateRateTransformations.getRateFromPointAtContainer(point, limitingContainer, coordinateSystemWhenDragStarted)
    ...
    function onUpstreamScaleChanged(event) {
        ...
        if (event.type === 'center dragged') {
            ...
            let newMaxRate = UI.projects.foundations.utilities.dateRateTransformations.getRateFromPointAtContainer(point, rateCalculationsContainer, coordinateSystem)
        ...
        if (event.type === 'bottom dragged') {
            ...
            let newMinRate = UI.projects.foundations.utilities.dateRateTransformations.getRateFromPointAtContainer(point, rateCalculationsContainer, coordinateSystem)
    ...
    function positioningPhysics() {
        ...
        upCorner = UI.projects.foundations.utilities.coordinateTransformations.transformThisPoint(upCorner, limitingContainer)
        bottonCorner = UI.projects.foundations.utilities.coordinateTransformations.transformThisPoint(bottonCorner, limitingContainer)
        ...
        thisObject.rate = UI.projects.foundations.utilities.dateRateTransformations.getRateFromPointAtBrowserCanvas(ratePoint, rateCalculationsContainer, coordinateSystem)
        ...
        ratePoint = UI.projects.foundations.utilities.coordinateTransformations.transformThisPoint(ratePoint, limitingContainer.frame.container)
        ...
        if (UI.projects.foundations.spaces.chartingSpace.viewport.zoomTargetLevel < UI.projects.foundations.globals.zoom.ZOOM_OUT_THRESHOLD_FOR_DISPLAYING_SCALES) {
            thisObject.isVisible = false
        }
    }
    ...
    function drawScale() {
        const SEPARATION = 50
        const NUMBER_OF_LABELS = Math.trunc(UI.projects.foundations.spaces.chartingSpace.viewport.height / SEPARATION)
        const FONT_SIZE = 15
        ...
        for (let i = 0; i <= NUMBER_OF_LABELS; i++) {
            ...
            let rate = UI.projects.foundations.utilities.dateRateTransformations.getRateFromPointAtBrowserCanvas(ratePoint1, rateCalculationsContainer, coordinateSystem)
    ...
    function drawScaleBox() {
        ...
        // This controls the color of the value/ rate scale panel on the right in the charts.
