                        ...
                        function addAllDataDependenciesIfNotExists(botDataDependencies) {
                            for (let l = 0; l < product.datasets.length; l++) {
                                ...
                                if (UI.projects.visualScripting.utilities.nodeChildren.isMissingChildrenById(botDataDependencies, dataset, true) === true) {
                                    savePluginFile = true
                                    let dataDependency = UI.projects.visualScripting.nodeActionFunctions.uiObjectsFromNodes.addUIObject(botDataDependencies, 'Data Dependency')
                                    UI.projects.visualScripting.nodeActionFunctions.attachDetach.referenceAttachNode(dataDependency, dataset)
                                }
                            }
                        }
                    }
                    ...
                    for (let i = 0; i < rootNodes.length; i++) {
                        ...
                        if (rootNode.type === 'Plugins') {
                            ...
                            for (let j = 0; j < tradingMinePlugin.pluginFiles.length; j++) {
                                ...
                                UI.projects.communityPlugins.nodeActionFunctions.pluginsFunctions.savePluginFile(pluginFile)
                            }
                        }
                    }
                    ...
                    for (let k = 0; k < projectDashboard.dashboards.length; k++) {
                        ...
                        for (let l = 0; l < dashboard.timeMachines.length; l++) {
                            ...
                            function createTimelineChartWithAllBranches(timeMachine) {
                                let newTimelineChart = UI.projects.visualScripting.nodeActionFunctions.uiObjectsFromNodes.addUIObject(timeMachine, 'Timeline Chart')
                                newTimelineChart.name = productDataMineParent.name

                                let layerManager = newTimelineChart.layerManager
                                UI.projects.visualScripting.nodeActionFunctions.attachDetach.referenceAttachNode(layerManager, dataMineProduct)
