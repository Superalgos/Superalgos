exports.newFoundationsBotModulesCandlesVolumesMultiTimeFrameDaily = function (processIndex) {
    ...
    Read the candles and volumes from Exchange Raw Data and produce a 
    file for each day and for each period with candles and volumes.
    */
    function start(callBackFunction) {
        try {
                ...
                try {
                    ...
                    if (thisReport.lastFile !== undefined) {
                        ...
                        if (beginingOfMarket.valueOf() !== contextVariables.datetimeBeginingOfMarketFile.valueOf()) { // Reset Mechanism for Begining of the Market
                            ...
                            beginingOfMarket = new Date(contextVariables.datetimeBeginingOfMarketFile.getUTCFullYear() + "-" + (contextVariables.datetimeBeginingOfMarketFile.getUTCMonth() + 1) + "-" + contextVariables.datetimeBeginingOfMarketFile.getUTCDate() + " " + "00:00" + TS.projects.foundations.globals.timeConstants.GMT_SECONDS);
                        ...
                        For that reason we go back one day, the partial information is discarded 
                        and added again with whatever new info is available.
                        */
                        ...
                        contextVariables.datetimeLastProducedFile = new Date(
                            ...
                            contextVariables.datetimeBeginingOfMarketFile.getUTCDate() + " " + "00:00" +
                            TS.projects.foundations.globals.timeConstants.GMT_SECONDS)
                        contextVariables.datetimeLastProducedFile = new Date(contextVariables.datetimeLastProducedFile.valueOf() - TS.projects.foundations.globals.timeConstants.ONE_DAY_IN_MILISECONDS); // Go back one day to start well.
                ...
                try {
                    ...
                    function timeframesLoop() {
                        ...
                        */
                        ...
                        function loopBody() {
                            const outputPeriod = TS.projects.foundations.globals.timeFrames.dailyTimeFramesArray()[n][0]
            ...
            function writeDataRanges(callBack) {
                try {
                    writeDataRange(contextVariables.datetimeBeginingOfMarketFile, contextVariables.datetimeLastProducedFile, CANDLES_FOLDER_NAME, onCandlesDataRangeWritten);
                    ...
                    function onCandlesDataRangeWritten(err) {
                        ...
                        writeDataRange(contextVariables.datetimeBeginingOfMarketFile, contextVariables.datetimeLastProducedFile, VOLUMES_FOLDER_NAME, onVolumeDataRangeWritten);
            ...
            function writeDataRange(pBegin, pEnd, pProductFolder, callBack) {
                try {
                    ...
                    let filePath =
                        TS.projects.foundations.globals.processVariables.VARIABLES_BY_PROCESS_INDEX_MAP.get(processIndex).FILE_PATH_ROOT + "/Output/" + pProductFolder + "/" +
                        TS.projects.foundations.globals.taskConstants.TASK_NODE.bot.processes[processIndex].referenceParent.config.codeName
                    filePath += '/' + fileName
